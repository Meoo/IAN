...
function importProject(self, prjName, filters, baseCriteria)
  ...
  local prj = sln.projects[prjName] -- Do not use findproject, may not work with virtual projects
  if not prj then
    premake.error("Project "..prjName.." is not in the solution "..sln.name, 0)
  end
...
function export(name)
  if type(name) == "table" then
    ...
    if #name > 1 then
      premake.error("export cannot take more than one argument", 1)
    end
  ...
  if type(name) ~= "string" then
    premake.error("export argument must be a string", 1)
  end
